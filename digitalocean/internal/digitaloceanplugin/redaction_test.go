...
func TestMissingCredentialGuidanceSurvivesRedaction(t *testing.T) {
	...
	for _, want := range []string{TokenEnvVar, "connector-secrets.yaml", "keychain://digitalocean/api_token", "managed load digitalocean"} {
...
func TestRefusalsSurviveRedaction(t *testing.T) {
	...
	result, err := callTool(p, "list_droplets", map[string]any{argDryRun: true})
