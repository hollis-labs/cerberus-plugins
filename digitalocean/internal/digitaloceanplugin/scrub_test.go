...
const sentinelToken = "SENTINEL-DO-TOKEN+/=4b7e19c3"
...
func TestErrorsNeverCarryTheToken(t *testing.T) {
	leaky := &fakeBackend{err: fmt.Errorf(
		`GET "https://api.digitalocean.com/v2/droplets?token=%s": 401 Unauthorized {"echo":"%s","header":"Bearer %s"}`,
...
func TestScrubberLeavesTextWithoutTheValueAlone(t *testing.T) {
	...
	const text = "list droplets: 403 Forbidden"
...
func TestScrubberIgnoresDegenerateValues(t *testing.T) {
	s := newScrubber("", "a")
	if got := s.text("a droplet named a"); got != "a droplet named a" {
