...
import (
	...
	"github.com/hollis-labs/cerberus-plugins/digitalocean/internal/digitaloceanplugin"
...
func main() {
	if len(os.Args) > 2 && os.Args[1] == "write-dist" {
		if err := digitaloceanplugin.WriteDist(os.Args[2]); err != nil {
			fmt.Fprintln(os.Stderr, "write-dist:", err)
			os.Exit(1)
		}
		return
	}
	if err := subprocess.Serve(digitaloceanplugin.New()); err != nil {
		os.Exit(1)
	}
}
