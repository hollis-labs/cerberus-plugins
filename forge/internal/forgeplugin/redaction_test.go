...
func TestMissingCredentialGuidanceSurvivesRedaction(t *testing.T) {
	...
	for _, want := range []string{TokenEnvVar, "connector-secrets.yaml", "keychain://forge/api_token", "managed load forge"} {
...
func TestRefusalsSurviveRedaction(t *testing.T) {
	...
	for op := range writeOperations {
		...
		_, message := callFailure(t, p, op, args)
		assertSurvivesRedaction(t, op+" acknowledgment refusal", message)
	}
	result, err := callTool(p, "list_servers", map[string]any{argDryRun: true})
