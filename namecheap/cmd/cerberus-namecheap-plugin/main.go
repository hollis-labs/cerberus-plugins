...
import (
	...
	"github.com/hollis-labs/cerberus-plugins/namecheap/internal/namecheapplugin"
...
func main() {
	if len(os.Args) > 2 && os.Args[1] == "write-dist" {
		if err := namecheapplugin.WriteDist(os.Args[2]); err != nil {
			fmt.Fprintln(os.Stderr, "write-dist:", err)
			os.Exit(1)
		}
		return
	}
	if err := subprocess.Serve(namecheapplugin.New()); err != nil {
		os.Exit(1)
	}
}
