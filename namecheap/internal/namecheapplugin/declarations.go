...
var suggestedPolicy = []cerbplugin.SuggestedRule{
	{Operation: "set_custom_nameservers", Require: cerbplugin.RequireApproval, Reason: "moves the domain's DNS to other nameservers"},
	{Operation: "set_dns_record_set", Require: cerbplugin.RequireApproval, Reason: "replaces every host record of the domain; records left out are deleted"},
}

// cliOnly operations never reach MCP, whatever connector-config.yaml says.
var cliOnly = []string{"set_dns_record_set"}
