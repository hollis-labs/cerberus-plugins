...
import (
	...
	"github.com/hollis-labs/cerberus-plugins/azure/internal/azplugin"
...
func main() {
	if len(os.Args) > 2 && os.Args[1] == "write-dist" {
		if err := azplugin.WriteDist(os.Args[2]); err != nil {
			fmt.Fprintln(os.Stderr, "write-dist:", err)
			os.Exit(1)
		}
		return
	}
	if err := subprocess.Serve(azplugin.New()); err != nil {
		os.Exit(1)
	}
}
